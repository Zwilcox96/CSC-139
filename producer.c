...
int main(int argc, char* argv[])
{
    ...
    if (bufSize > 1000) {
        printf("Buffer size exceeds accepted input (<=1000)");
        exit(1);
    }
    if (itemCnt < 1) {
        printf("You must make at least 1 item");
        exit(1);
    }
...
void InitShm(int bufSize, int itemCnt)
{
    ...
    int out = 0;
    int shared_fd;
	const char *name = "OS_HW1_ZackWilcox"; // Name of shared memory object to be passed to shm_open
     ...
     gShmPtr = mmap(0, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shared_fd, 0);
